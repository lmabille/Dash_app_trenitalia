...
for stop in stop_in_station:
    if stop['delay']:
        if dict_ID_StationName[stop['current_station']] in dict.keys():
            dict[dict_ID_StationName[stop['current_station']]] += stop['delay']
        else:
            dict[dict_ID_StationName[stop['current_station']]] = stop['delay']
...
df = pd.DataFrame(list(dict.items()), columns=['station_name', 'delay_minutes'])
df.to_csv('station_biggest_delay_minutes.csv', index=False)
# Créer un graphique en bar avec plotly express
fig = px.bar(df, x='station_name', y='delay_minutes')
